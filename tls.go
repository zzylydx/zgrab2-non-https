...
// 将自己程序的配置传送给用于连接的真实包，也就是tls.Config
...
// 修改，添加新字段，SCTlog，用于存放三种sct传递字段
type TLSLog struct {
	// TODO include TLSFlags?
	SCTLog *tls.SignedCertificateTimestampLog `json:"sct_log"`
...
func (t *TLSFlags) GetTLSConnectionForTarget(conn net.Conn, target *ScanTarget) (*TLSConnection, error) {
	cfg, err := t.GetTLSConfigForTarget(target) //将client的配置传递给配置给真实的包
	if err != nil {
...
func (t *TLSFlags) GetWrappedConnection(conn net.Conn, cfg *tls.Config) *TLSConnection {
	tlsClient := tls.Client(conn, cfg) //发起tls握手
