...
func ZGrab2Main() {
	...
	_, moduleType, flag, err := zgrab2.ParseCommandLine(os.Args[1:]) //读入命令，并初始化配置，详见config.go
	...
	if err != nil {
		...
		log.Fatalf("could not parse flags: %s", err) // print err,然后 call system exit
	}
	...
	if m, ok := flag.(*zgrab2.MultipleCommand); ok {
		...
		if len(modTypes) != len(flagsReturned) { //判断文件读入的类型和标识是否一对一
	...
	wg := sync.WaitGroup{}
	monitor := zgrab2.MakeMonitor(1, &wg) //收集和发送正在运行的扫描状态，并将这些结果传递给监视器的通道
	monitor.Callback = func(_ string) { //看不懂
		dumpHeapProfile()
	}
	start := time.Now()
	log.Infof("started grab at %s", start.Format(time.RFC3339))
	zgrab2.Process(monitor) //Process设置输出编码器，输入读取器，然后启动抓取程序，并将结果写入输出通道
	end := time.Now() //结束时间
