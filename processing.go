...
import (
	...
	"github.com/zzylydx/zgrab2/lib/output"
)
...
// Open使用已配置的标志连接到ScanTarget，并返回一个net.Conn，该网络使用已配置的超时进行ReadWrite操作
func (target *ScanTarget) Open(flags *BaseFlags) (net.Conn, error) {
	...
	address := net.JoinHostPort(target.Host(), fmt.Sprintf("%d", port)) // 主机+端口
...
func (target *ScanTarget) OpenUDP(flags *BaseFlags, udp *UDPFlags) (net.Conn, error) {
	...
	address := net.JoinHostPort(target.Host(), fmt.Sprintf("%d", port)) //返回主机+端口
...
//EncodeGrab将Grab序列化为JSON，并在必要时处理调试字段
func EncodeGrab(raw *Grab, includeDebug bool) ([]byte, error) {
	...
	if includeDebug {
		...
		// 如果调用方未明确请求调试数据，则将其删除
		// TODO: Migrate this to the ZMap fork of sheriff, once it's more stable.
	...
	return json.Marshal(outputData) // 将数据序列化，调用这个接口，使用每个结构体的Marshal方法序列化数据为json，格式为byte
}
...
func grabTarget(input ScanTarget, m *Monitor) []byte {
	...
	for _, scannerName := range orderedScanners {
		...
		moduleResult[name] = res // 将返回数据存入moduleResult
	...
	//抓取结果
	raw := BuildGrabFromInputResponse(&input, moduleResult) //grab构成
	result, err := EncodeGrab(raw, includeDebugOutput()) //编码为字节
	if err != nil {
...
func Process(mon *Monitor) {
	...
	// Start the output encoder，将结果从通道写入writer
	go func() {
		...
		go func(i int) { //开启协程
			...
			for obj := range processQueue {
				for run := uint(0); run < uint(config.ConnectionsPerHost); run++ {
					result := grabTarget(obj, mon) //夹杂监视器和扫描对象，开始扫描，并将结果抓取下来
					outputQueue <- result //将扫描结果输入输出通道
				}
			}
			workerDone.Done()
		}(i)
