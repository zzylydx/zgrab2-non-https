...
type ScanResults struct {
	// Banner is the string sent by the server immediately after connecting. 服务器发送的数据
	...
	// to using StartTls
	ImplicitTLS bool `json:"implicit_tls,omitempty"` //tls
...
// RegisterModule注册zgrab2模块，将命令添加进去
func RegisterModule() {
	var module Module
...
func (scanner *Scanner) Scan(target zgrab2.ScanTarget) (zgrab2.ScanStatus, interface{}, error) {
	...
	if err != nil {
		return zgrab2.TryGetScanStatus(err), nil, err // 出现问题，将错误归类
	}
	defer c.Close() // 推迟连接断开，为后续tls连接放入数据
	result := &ScanResults{} // 存放扫描结果的结构体
	if scanner.config.SMTPSecure {  // 1、使用smtps连接
		tlsConn, err := scanner.config.TLSFlags.GetTLSConnection(c) // 配置好tls然后发起tls连接
		if err != nil {
			return zgrab2.TryGetScanStatus(err), nil, err
		}
		result.TLSLog = tlsConn.GetLog()
		if err := tlsConn.Handshake(); err != nil { // 发起握手
			return zgrab2.TryGetScanStatus(err), result, err
		}
		c = tlsConn
		result.ImplicitTLS = true // 代表使用smtps连接，而不是starttls
	}
	conn := Connection{Conn: c}
	banner, err := conn.ReadResponse() // 读取连接中的数据
	if err != nil {
		if !scanner.config.SMTPSecure {
			result = nil
		}
		return zgrab2.TryGetScanStatus(err), result, err // 返回数据
	}
	// Quit early if we didn't get a valid response
	// OR save response to return later
	sr, bannerResponseCode := VerifySMTPContents(banner) // 验证是否出错
	...
	if scanner.config.SendEHLO {
		ret, err := conn.SendCommand(getCommand("EHLO", scanner.config.EHLODomain)) // 传输命令，并存储返回内容
		if err != nil {
			return zgrab2.TryGetScanStatus(err), result, err
		}
		result.EHLO = ret
	...
	if scanner.config.StartTLS { // 2、使用starttls连接
		ret, err := conn.SendCommand("STARTTLS")
		if err != nil {
			return zgrab2.TryGetScanStatus(err), result, err
		}
		result.StartTLS = ret // 将内容存储
		...
		tlsConn, err := scanner.config.TLSFlags.GetTLSConnection(conn.Conn) //发送完starttls后，开始tls连接
		if err != nil {
			return zgrab2.TryGetScanStatus(err), result, err
		}
		result.TLSLog = tlsConn.GetLog()
	...
	if scanner.config.SendQUIT { // 3、最后发送QUIT，结束连接
